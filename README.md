...
- **Added 2026-08-17:** dereferenced the `era:netReference` targets that were previously inert leaves, following the real chain era:NetLinearReference → era:NetPointReference → era:TopologicalCoordinate → era:LinearElement (plus era:NetAreaReference and era:NetPointReference as area/point siblings) — all real data fetched live from `graph.data.era.europa.eu`, genuinely connected to Track ØB2's own linear location, with the LinearElement reusing the same 2019 validity record as its parent track.
